...
import com.imengyu.vr720.utils.NavigationBarUtils;
...
public class PanoActivity extends AppCompatActivity {
    ...
    private void initRenderer() {
        ...
        renderer.setProp(NativeVR720Renderer.PROP_GYRO_ROTATE_CORRECTION, screenExifOrientation);
    ...
    private void initView() {
        ...
        viewBinder.button_video_play_pause.setOnClickListener((v) -> switchVideoState());
    ...
    private void initStatusBar() {
        ...
        if(isLandscape) {
            //底栏顶高度都可以为0
            ViewGroup.LayoutParams lp = viewBinder.view_bottom_bar_placeholder.getLayoutParams();
            lp.height = 1;
            viewBinder.view_bottom_bar_placeholder.setLayoutParams(lp);
            lp = viewBinder.view_top_bar_placeholder.getLayoutParams();
            lp.height = showStatusBar ? StatusBarUtils.getStatusBarHeight(this) : 1;
            viewBinder.view_top_bar_placeholder.setLayoutParams(lp);

            //横向就把边距移到左右

            //刘海屏高度
            int notchSize = NotchScreenUtil.getNotchSize(this);
            int navBarSize = NavigationBarUtils.getNavigationBarHeight(this);

            if(screenExifOrientation == ExifInterface.ORIENTATION_NORMAL) //左
                viewBinder.activity_pano.setPadding(notchSize,0, navBarSize,0);
            else if(screenExifOrientation == ExifInterface.ORIENTATION_ROTATE_180) //右
                viewBinder.activity_pano.setPadding(navBarSize,0,notchSize,0);
        }
        else {

            viewBinder.activity_pano.setPadding(0,0,0,0);
            ...
            if (showStatusBar) {
                ...
                int topBarHeight = StatusBarUtils.getStatusBarHeight(this);
                ...
                if (NotchScreenUtil.hasNotchScreen(this)) {
                    ...
                    //刘海屏高度
                    int notchSize = NotchScreenUtil.getNotchSize(this);
                    if (notchSize > 0 && notchSize > topBarHeight)
                        topBarHeight = notchSize;
                }

                ViewGroup.LayoutParams lp = viewBinder.view_top_bar_placeholder.getLayoutParams();
            ...
            //底栏高度
            ViewGroup.LayoutParams lp = viewBinder.view_bottom_bar_placeholder.getLayoutParams();
            lp.height = NavigationBarUtils.getNavigationBarHeight(this);
            viewBinder.view_bottom_bar_placeholder.setLayoutParams(lp);
        }
    }
    ...
    private boolean keepScrennOn = false;
    ...
    private void setToolBarStatus(boolean show) {
        ...
        if (toolbarOn) {
            ...
            viewBinder.view_bottom_bar_placeholder.startAnimation(viewBinder.fade_show);
            viewBinder.view_bottom_bar_placeholder.setVisibility(View.VISIBLE);
            viewBinder.view_top_bar_placeholder.startAnimation(viewBinder.fade_show);
            viewBinder.view_top_bar_placeholder.setVisibility(View.VISIBLE);
            ...
            viewBinder.view_bottom_bar_placeholder.startAnimation(viewBinder.fade_hide);
            viewBinder.view_bottom_bar_placeholder.setVisibility(View.INVISIBLE);
            viewBinder.view_top_bar_placeholder.startAnimation(viewBinder.fade_hide);
            viewBinder.view_top_bar_placeholder.setVisibility(View.INVISIBLE);
    ...
    public void onBackPressed() {
        if(panoInfoShow) setPanoInfoStatus(false, true);
        else if(panoMenuShow) setPanoMenuStatus(false, true);
    ...
    private void onUpdateScreenOrientation(Configuration newConfig) {
        if(newConfig.orientation == Configuration.ORIENTATION_PORTRAIT) {
            ...
            mConstraintSet.connect(R.id.pano_menu, ConstraintSet.RIGHT, ConstraintSet.PARENT_ID, ConstraintSet.RIGHT, PixelTool.dp2px(this, 44));
    ...
    private int screenExifOrientation = ExifInterface.ORIENTATION_NORMAL;

    private boolean orientationChangedTick = true;
    private OrientationEventListener screenOrientationEventListener = null;
    ...
    private void initSensor() {
        ...
        screenOrientationEventListener = new OrientationEventListener(this) {
            @Override
            public void onOrientationChanged(int i) {
                if(45 <= i && i < 135) {
                    screenExifOrientation = ExifInterface.ORIENTATION_ROTATE_180;
                } else if(135 <= i && i < 225) {
                    screenExifOrientation = ExifInterface.ORIENTATION_ROTATE_270;
                } else if(225 <= i && i < 315) {
                    screenExifOrientation = ExifInterface.ORIENTATION_NORMAL;
                } else {
                    screenExifOrientation = ExifInterface.ORIENTATION_ROTATE_90;
                }
                if(orientationChangedTick) {
                    orientationChangedTick = false;
                    renderer.setProp(NativeVR720Renderer.PROP_GYRO_ROTATE_CORRECTION, screenExifOrientation);
                }
            }
        };
